...
export default function exerciceHoraireDebut(app: HTMLElement) {
  ...
  divStart.style.boxShadow = '0 1px 3px 0 rgb(0 0 0 / 0.1), 0 1px 2px -1px rgb(0 0 0 / 0.1)'
  ...
  divStart.style.backgroundColor = 'white'
  ...
  divStart.style.padding = '5px'
  divMiddle.style.padding = '5px'
  divMiddle2.style.padding = '5px'
  divEnd.style.padding = '5px'
  divStep1.style.padding = '5px'
  divStep2.style.padding = '5px'
  divStep3.style.padding = '5px'
  ...
  function drawLines (): void {
    if (numberOfSteps === 1) {
      line1 = new LeaderLine(divMiddle, divStart, { dropShadow: true })
      line2 = new LeaderLine(divEnd, divMiddle, { dropShadow: true })
      lineTotal = new LeaderLine(divEnd, divStart, { path: 'arc', dropShadow: true, startSocket: 'bottom', endSocket: 'bottom' })
    } else {
      line1 = new LeaderLine(divMiddle, divStart, { dropShadow: true })
      line2 = new LeaderLine(divMiddle2, divMiddle, { dropShadow: true })
      line3 = new LeaderLine(divEnd, divMiddle2, { dropShadow: true })
      lineTotal = new LeaderLine(divEnd, divStart, { path: 'arc', dropShadow: true, startSocket: 'bottom', endSocket: 'bottom' })
    }
  ...
  function initValues(): void {
    ...
    divEnonce.innerText = `Un évènement termine à ${endHour.toString()} et a duré ${endHour.substract(startHour).toString()}. À quelle heure a-t-il commencé ?`
    divStart.innerText = ''
  ...
  divStart.addEventListener('focusout', checkAllStep)
  ...
  function checkStep(divStart: HTMLElement, divStep: HTMLElement, divEnd: HTMLElement): boolean {
    ...
    if (
      ...
      if (divStep.innerText !== '+/- ?') divStep.style.color = 'green'
      return true
    } else {
